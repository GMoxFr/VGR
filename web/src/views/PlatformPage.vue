<template>
    <div v-if="platform" class="platform-container">
        <div class="platform-header-infos">
        ...
        <div v-if="games.length > 0" class="platform-details">
            <h2>🎮 Jeux disponibles sur cette plateforme</h2>
...
// Charger les informations de la plateforme avec la pagination
const fetchGames = async () => {
    try {
        console.log("Chargement des jeux pour la plateforme:", route.params.platformId);
        const response = await api.platform.get(route.params.platformId, page.value, maxResults);
...
const nameWithCount = computed(() => {
    return `${platform.value?.name} (${gameCount.value})`;
});

watch(page, fetchGames);
onMounted(fetchGames);

document.title = "VGR - Plateforme";
watch(platform, () => {
    if (platform.value) {
        document.title = `VGR - ${platform.value.name}`;
    }
});
</script>
