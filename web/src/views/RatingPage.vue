<template>
    <div v-if="rating" class="ratings-container">
        <div class="ratings-header-infos">
            <div class="ratings-header-info">
                <h1>{{ nameWithCount }}</h1>
            </div>
        </div>

        <!-- Liste des jeux avec cette note -->
        <div v-if="games.length > 0" class="ratings-details">
            <h2>🎮 Jeux avec cette évaluation</h2>
        ...
        <p v-if="games.length === 0" class="empty-message">
            Aucun jeu trouvé avec cette note.
        </p>
    </div>
</template>
...
// Charger les informations des jeux avec cette note avec la pagination
const fetchGames = async () => {
    try {
        console.log("Chargement des jeux pour la note:", route.params.ratingId);
        const response = await api.rating.get(route.params.ratingId, page.value, maxResults);
        rating.value = response.data;
        games.value = response.data.games;
        gameCount.value = response.data.totalGames;
    } catch (error) {
        console.error("Erreur lors du chargement des données de la note:", error);
    }
};
...
const nameWithCount = computed(() => {
    return `Note ${rating.value?.name} (${gameCount.value})`;
});
